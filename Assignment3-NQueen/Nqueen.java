...
public class Nqueen {
	 ...
	 * @param n represents the size of the board
		...
		if ( row >= n){
			...
			for ( int column = 0 ; column < n ; column++ ) {
				...
				if ( isSafe( board, row, column, n) ) {
					board[ row ][ column ] = 1 ; //storing 1 in the current column indicates that queen is placed in that square
					if ( nQueen ( board, row + 1, n) ) {
						return true; //returns true if the queen can be safely placed
					}
					board[ row ][ column ] = 0 ; //stores 0 again if the current position of queen is not safe during backtracking
				}
			}
